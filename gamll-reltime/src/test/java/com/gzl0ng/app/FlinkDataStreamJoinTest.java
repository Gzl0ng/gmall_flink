...
import com.gzl0ng.bean.Bean1;
import com.gzl0ng.bean.Bean2;
...
public class FlinkDataStreamJoinTest {
    public static void main(String[] args) throws Exception {
        ...
        //2.读取二个端口数据创建流并提取时间戳并生成watermark
        SingleOutputStreamOperator<Bean1> stream1 = env.socketTextStream("flink1", 8888)
                .map(line -> {
        ...
        SingleOutputStreamOperator<Bean2> stream2 = env.socketTextStream("flink1", 9999)
                .map(line -> {
                    String[] fields = line.split(",");
...
//                .lowerBoundExclusive()  排除左边界
//                .upperBoundExclusive()  排除右边界，不加这二个参数默认是1左闭右闭
